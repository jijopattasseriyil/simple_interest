...
function App() {
  ...
  const getValidate = (e)=>{
    ...
    if(!!value.match(/^[0-9]*.?[0-9]+$/)) {  // !! to convert into true or false statement
      if(name==='principle')
    {setPrinciple(value )
    setIsPrinciple(true)}
    else if(name==='rate'){
       ...
       <div className='bg-light p-5 rounded'>
        ...
        <div className='bg-warning d-flex justify-content-center align-items-center w-100 p-3 flex-column rounded mt-4 shadow'>
         ...
         <form className='mt-4' onSubmit={handleCalculate}>
            ...
            <div className='mb-3'>
            <TextField name='year' value={year || ""} className='w-100' onChange={(e)=>getValidate(e)}  id="outlined-basic" label="Year (Yr)" variant="outlined" />
            </div>
            {
              !isYear &&
            <div>
